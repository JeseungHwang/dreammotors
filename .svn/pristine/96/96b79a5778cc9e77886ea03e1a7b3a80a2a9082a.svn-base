...
<body>
				  ...
				  <ul class="pagination">
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="pubrd_search.do?prepage=<%=pVO.getPrevPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				    	<%}else{%>
				    ...
				    <%	int pgCnt=0, endPage = pVO.getEndPageNo();
						...
						<%}else{ %>
							<% if((String)request.getAttribute("SEARCH")!=null){%>
								<li id="paging"><a href="pubrd_search.do?page=<%=i%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>"><%= i %></a></li>
							<%}else{ %>
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="pubrd_search.do?nextpage=<%=pVO.getNextPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
				    	<%}else{%>
	...
	function brdsearch(){
		submitPost('${pageContext.request.contextPath}/pubrd_search.do', {'stype':$('select[name="inpStype"]').val(), 'sCntnt':$('input[name=inpScntnt]').val()});
	}
	
	function insertBoard() {
		location.href = "${pageContext.request.contextPath}/pubrd_wpg.do";
	}
	</script>
</html>