...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 고객 상담 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 중고차 구입 문의 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 상세내용</label>
		</div>
		...
		<div id="right-conentform">
			...
			<div class="pboard-cntnt-cmt-write">
				...
				<div class="pboard-cmt-wrap">
					...
					<div class="btn_cmtadd"><input type="button" class="btn btn-default cmt-btn-insert" value="등록" onclick="cmt_insert()"/></div>
				</div>			
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function btn_brdModify(){
			<% if((String)session.getAttribute("UID") != null){%>
				submitPost('${pageContext.request.contextPath}/pubrd_comparePW.do', {'index':<%= board.getBoardIndex()%>});
			<%}%>
			$('.btn_brdModify').hide();
			$('.brdModify_form').show();
			$('.brdDelete_form').hide();
		...
		function btn_brdDelete(){
			<% if((String)session.getAttribute("UID") != null){%>
				if(!confirm("해당 글을 삭제하시겠습니까?")){
					...
					submitPost('${pageContext.request.contextPath}/pubrd_delete.do', {'index':<%= board.getBoardIndex()%>});
				}
			<%}%>
			$('.btn_brdModify').hide();
			$('.brdModify_form').hide();
		...
		function btn_modify_comparePW(){
			submitPost('${pageContext.request.contextPath}/pubrd_comparePW.do', {'index':<%= board.getBoardIndex()%>,'brdPW':$("input#inpModifyBrdPW").val()});
		}
		function btn_delete_comparePW(){
			if(!confirm("해당 글을 삭제하시겠습니까?")){
				return
			}else{
				submitPost('${pageContext.request.contextPath}/pubrd_delete.do', {'index':<%= board.getBoardIndex()%>,'brdPW':$("input#inpDeleteBrdPW").val()});
			}
		}
		function cmt_insert(){
			 if($.trim($('input#cmt-wRegNm').val()) == ""){
				 ...
				 submitPost('${pageContext.request.contextPath}/pucomment_add.do', {'index':<%= board.getBoardIndex()%>,'regNm':$("input#cmt-wRegNm").val(),'cmt':$("textarea#cmt-wCmt").val() }); 
			 }
		}
	</script>
