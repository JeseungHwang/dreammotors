...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 고객 상담 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 중고차 판매문의</label>
		</div>
					...
					<tr id="<%=list.getBoardIndex()%>">
						...
						<td><%=list.getBoardTitle()%>
							<% if(list.getBoardIsPrivate()==1){
								%> <span class="glyphicon glyphicon-lock" id="private_icon" aria-hidden="true" style="color:gray;"></span><% 
							}%>
				  ...
				  <ul class="pagination">
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="sabrd_search.do?prepage=<%=pVO.getPrevPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				    	<%}else{%>
				    ...
				    <%	int pgCnt=0, endPage = pVO.getEndPageNo();
						...
						<%}else{ %>
							<% if((String)request.getAttribute("SEARCH")!=null){%>
								<li id="paging"><a href="sabrd_search.do?page=<%=i%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>"><%= i %></a></li>
							<%}else{ %>
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="sabrd_search.do?nextpage=<%=pVO.getNextPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
				    	<%}else{%>
	...
	function brdsearch(){
		submitPost('${pageContext.request.contextPath}/sabrd_search.do', {'stype':$('select[name="inpStype"]').val(), 'sCntnt':$('input[name=inpScntnt]').val()});
	}
	function insertBoard() {
		location.href = "${pageContext.request.contextPath}/sabrd_wpg.do";
	}
	</script>
</html>