...
<%if(request.getAttribute("CHECK")!=null){
	if(request.getAttribute("CHECK").equals("isu")){%>
		...
		location.href = "${pageContext.request.contextPath}/sabrd.do"; </script>
	<%}
}%>
...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 고객 상담 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 중고차 판매 문의 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 글작성</label>
		</div>
		...
		<div id="right-conentform">
			<form action="${pageContext.request.contextPath}/sabrd_write.do" class="form-horizontal" id="saboard-writeForm" method="POST">
				...
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wTitle" id="saboard-wTitle" name="inpTitle" autofocus>
				...
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wNm" id="saboard-wNm" name="inpNm" <%if(request.getAttribute("RESULT") != null){%>value="<%=vo.getMemName()%>"<%}%>>
						<p id="sab-nm-help"></p>
					</div>
				</div>
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wTell" id="saboard-wTell" placeholder="- (하이픈) 없이 입력해주세요" name="inpTell" maxlength="11" <%if(request.getAttribute("RESULT") != null){%>value="<%=vo.getMemTell()%>"<%}%>>
				...
				<div id="brd-form-group1">
					...
					<div id="brd-context">
						<textarea name="inpContent" id="saboard-wCntnt" rows="10" cols="100" style="width:100%; height:412px; display:none;"></textarea>
					</div>
				</div>
				<div id="brd-form-group">
					<div id="brd-label-form"><span id="brd-label">비밀번호</span></div>
					<div id="brd-context">
						<input type="password" class="brd-wPw" id="saboard-wPw" name="inpPw">
						<p id="sab-pw-help"></p>
					</div>
				</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('#saboard-writeForm').submit(function(ev) {
			    ...
			    if($.trim($('input#saboard-wPw').val()) == ""){
			    	$('p#sab-pw-help').html('비밀번호를 입력해주세요.').css('color','red');
			    	isEmpty[3] = false;
			    }else{
			    	isEmpty[3] = true;
			    }
			    if(isEmpty[0] && isEmpty[1] && isEmpty[2] && isEmpty[3]){
			    	oEditors.getById["saboard-wCntnt"].exec("UPDATE_CONTENTS_FIELD", []);
