...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 커뮤니티 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 차량 판매 후기<span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 글수정</label>
		</div>
		...
		<div id="right-conentform">
			<form action="${pageContext.request.contextPath}/rebrd_modify.do" class="form-horizontal" id="rebrd-modifyForm" method="POST">
				<div id="brd-form-group">
					<div id="brd-label-form"><span id="brd-label">제목</span></div>
					<div id="brd-context">
						<input type="text" class="brd-wTitle" id="reboard-wTitle" name="inpTitle" autofocus value="<%= board.getBoardTitle()%>">
				...
				<div id="brd-form-group1">
					...
					<div id="brd-context">
						<textarea name="inpContent" id="rebrd-modify-content" rows="10" cols="100" style="width:100%; height:412px; display:none;"><%= board.getBoardContext()%></textarea>
					</div>
				</div>
	...
	$(document).ready(function(){	
		$('#rebrd-modifyForm').submit(function(ev) {
			 ...
			 var isEmpty=false;
			    ...
			    if(isEmpty){
			    	oEditors.getById["rebrd-modify-content"].exec("UPDATE_CONTENTS_FIELD", []);
		...
		nhn.husky.EZCreator.createInIFrame({
			oAppRef : oEditors,
			elPlaceHolder : "rebrd-modify-content",
