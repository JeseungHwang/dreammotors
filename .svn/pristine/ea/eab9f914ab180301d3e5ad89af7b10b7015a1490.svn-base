...
<body>
	...
	<div class="container">
		...
		<div id="right-conentform">
			<form action="${pageContext.request.contextPath}/pubrd_modify.do" class="form-horizontal" id="puboard-modifyForm" method="POST">
				<div id="brd-form-group">
					<div id="brd-label-form"><span id="brd-label">분류</span></div>
					<div id="brd-context">
						<select class="brd-btype" name="inpBtype" id="pubrd-modify-btype">
				...
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wTitle" id="pubrd-modify-title" name="inpTitle" value="<%= board.getBoardTitle()%>">
						<div class="checkbox" id="brd-isprivate">
							<span id="brd-label2">
								<% if(board.getBoardIsPrivate() == 1){ %>
									<input type="checkbox" id="pubrd-modify-isprivate" name="isPrivate" checked>
								<%}else{ %>
									<input type="checkbox" id="pubrd-modify-isprivate" name="isPrivate">
								<%} %>
								<strong>비밀글</strong>
							</span>
						</div>
				...
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wNm" id="pubrd-modify-nm" name="inpNm" value="<%= board.getBoardRegName()%>">
						<p id="sab-nm-help"></p>
					</div>
				</div>
				<div id="brd-form-group">
					...
					<div id="brd-context">
						<input type="text" class="brd-wTell" id="pubrd-modify-tell" placeholder="- (하이픈) 없이 입력해주세요" name="inpTell" maxlength="11" value="<%= board.getBoardTell()%>">
				...
				<div id="brd-form-group1">
					...
					<div id="brd-context">
						<textarea name="inpContent" id="pubrd-modify-content" rows="10" cols="100" style="width:100%; height:412px; display:none;"><%= board.getBoardContext()%></textarea>
					</div>
				</div>
				<div id="brd-form-group2">
					<input class="brd-btn btn btn-danger" type="button" value="수정" onclick="pubrd_modifyEvent()" />
	...
	<script type="text/javascript">
		function pubrd_modifyEvent(){
		    ...
		    if(isEmpty[0] && isEmpty[1] && isEmpty[2]){
		    	oEditors.getById["pubrd-modify-content"].exec("UPDATE_CONTENTS_FIELD", []);
		    	submitPost('${pageContext.request.contextPath}/pubrd_modify.do', 
		    			{'index':<%= board.getBoardIndex()%>,
			    		'inpBtype':$('#pubrd-modify-btype').val(),
			    		'inpIsPrivate':$("input:checkbox[id='pubrd-modify-isprivate']").is(":checked"),
			    		'inpTitle':$('#pubrd-modify-title').val(),
			    		'inpNm':$('#pubrd-modify-nm').val(),
			    		'inpTell':$('#pubrd-modify-tell').val(),
			    		'inpContent':$('#pubrd-modify-content').val()});
		    	alert("성공적으로 수정되었습니다.");
		    }
		}
		var oEditors = [];
		nhn.husky.EZCreator.createInIFrame({
		 oAppRef: oEditors,
		 elPlaceHolder: "pubrd-modify-content",
