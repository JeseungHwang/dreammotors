...
<%if(request.getAttribute("CHECK")!=null){
	if(request.getAttribute("CHECK").equals("isu")){%>
		...
		location.href = "${pageContext.request.contextPath}/nobrd.do"; </script>
	<%}
}%>
...
<body>
	...
	<div class="container">
		...
		<div id="right-conentform">
			<form action="${pageContext.request.contextPath}/nobrd_write.do" class="form-horizontal" id="nobrd-writeForm" method="POST">
				<div id="brd-form-group">
					<div id="brd-label-form"><span id="brd-label">제목</span></div>
					<div id="brd-context">
						<input type="text" class="brd-wTitle" id="nobrd-wTitle" name="inpTitle" autofocus>
						<div class="checkbox" id="brd-isnotice">
							<span id="brd-label2"><input type="checkbox" name="isNotice" value="check"><strong>공지</strong></span>
						</div>
				...
				<div id="brd-form-group1">
					...
					<div id="brd-context">
						<textarea name="inpContent" id="nobrd-wCntnt" rows="10" cols="100" style="width:100%; height:412px; display:none;"></textarea>
					</div>
				</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){	
			$('#nobrd-writeForm').submit(function(ev) {
			    ...
			    if(isEmpty){
			    	oEditors.getById["nobrd-wCntnt"].exec("UPDATE_CONTENTS_FIELD", []);
