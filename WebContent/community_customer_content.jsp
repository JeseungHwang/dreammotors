...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 커뮤니티 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 고객 후기 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 상세내용</label>
		</div>
	...
	<script type="text/javascript">
		...
		function btn_brdModify(){
			<% if((String)session.getAttribute("UID") != null){%>
				submitPost('${pageContext.request.contextPath}/cubrd_comparePW.do', {'index':<%= board.getBoardIndex()%>});
			<%}else{%>	
				$('.btn_brdModify').hide();
				$('.brdModify_form').show();
				$('.brdDelete_form').hide();
		...
		function btn_brdDelete(){
			<% if((String)session.getAttribute("UID") != null){%>
				if(!confirm("해당 글을 삭제하시겠습니까?")){
					...
					submitPost('${pageContext.request.contextPath}/cubrd_delete.do', {'index':<%= board.getBoardIndex()%>});
				}
			<%}else{%>
				$('.btn_brdModify').hide();
				$('.brdModify_form').hide();
		...
		function btn_modify_comparePW(){
			submitPost('${pageContext.request.contextPath}/cubrd_comparePW.do', {'index':<%= board.getBoardIndex()%>,'brdPW':$("input#inpModifyBrdPW").val()});
		}
		function btn_delete_comparePW(){
			if(!confirm("해당 글을 삭제하시겠습니까?")){
				return
			}else{
				submitPost('${pageContext.request.contextPath}/cubrd_delete.do', {'index':<%= board.getBoardIndex()%>,'brdPW':$("input#inpDeleteBrdPW").val()});
			}
		}
		function cmt_insert(){
			 if($.trim($('input#cmt-wRegNm').val()) == ""){
				 ...
				 submitPost('${pageContext.request.contextPath}/cucomment_add.do', {'index':<%= board.getBoardIndex()%>,'regNm':$("input#cmt-wRegNm").val(),'cmt':$("textarea#cmt-wCmt").val() }); 
			 }
		}
	</script>
</html>
