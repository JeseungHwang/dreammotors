...
<body>
	...
	<div class="container">
		...
		<div class="directory-label">
			<label class="control-label"><span class="glyphicon glyphicon-home" aria-hidden="true"></span> 커뮤니티 <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> 차량 판매 후기</label>
		</div>
				  ...
				  <ul class="pagination">
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="rebrd_search.do?prepage=<%=pVO.getPrevPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				    	<%}else{%>
				    ...
				    <%	int pgCnt=0, endPage = pVO.getEndPageNo();
						...
						<%}else{ %>
							<% if((String)request.getAttribute("SEARCH")!=null){%>
								<li id="paging"><a href="rebrd_search.do?page=<%=i%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>"><%= i %></a></li>
							<%}else{ %>
				    ...
				    <%}else{ %>
				    	<% if((String)request.getAttribute("SEARCH")!=null){%>
				    		<li><a href="rebrd_search.do?nextpage=<%=pVO.getNextPageNo()%>&sCntnt=<%=(String)request.getAttribute("SEARCH")%>&stype=<%=(String)request.getAttribute("STYPE")%>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
				    	<%}else{%>
	...
	function brdsearch(){
		submitPost('${pageContext.request.contextPath}/rebrd_search.do', {'stype':$('select[name="inpStype"]').val(), 'sCntnt':$('input[name=inpScntnt]').val()});
	}
	function insertBoard() {
		location.href = "${pageContext.request.contextPath}/rebrd_wpg.do";
	}
	</script>
</html>